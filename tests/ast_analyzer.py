#!/usr/bin/env python3
"""
AST Analyzer Script - Analyzes a repository and saves its AST to a file
that can be used by core2/auditor.py
"""
...
class CodeParser:
    def __init__(self, lang: str, repo_root: Path):
        ...
        try:
            ...
            print(f"Tree-sitter parser not available for '{lang}': {e}")
            self.parser = None
            self.lib = None
    ...
    def parse_file(self, path: Path, content: bytes) -> Tuple[List[Node], List[Edge]]:
        ...
        try:
            ...
            root = tree.root_node
            ast_dump = dump_ast(root, content)
            with open(f"ast_{path.name}.txt", "w", encoding="utf-8") as f:
                f.write("\n".join(ast_dump))
        except Exception as e:
...
def dump_ast(node, source, indent=0, out=None):
    if out is None:
        out = []
    pad = "  " * indent
    txt = source[node.start_byte:node.end_byte].decode("utf-8", errors="ignore")
    txt = txt.replace("\n", "\\n")
    out.append(
        f"{pad}{node.type} "
    ...
    for c in node.children:
        dump_ast(c, source, indent + 1, out)
    return out
        ...
        if self.out_dir:
            ...
            if existing:
                ...
                if code_hash in self._code_hash_to_file:
                    ...
                    print(f"Added path to existing trace file {existing_trace_id} (code_hash: {code_hash[:8]}...)")
            else:
...
def main():
    ap = argparse.ArgumentParser(description="Analyze a repository and save its AST to a file")
    ap.add_argument("--repo", required=True, help="Path to repository to analyze")
    ap.add_argument("--output", required=True, help="Output file path to save the AST data")
    ap.add_argument("--format", choices=["json", "pickle"], default="json", 
                   help="Output format (json or pickle)")
    ...
    # Parse all files and extract nodes and edges
    all_nodes = []
    all_edges = []
    ...
    for lang, paths in files_map.items():
        ...
        for idx, p in enumerate(paths):
            try:
                ...
                # Convert nodes to dictionaries for serialization
                for node in nodes:
                    all_nodes.append({
                        "uid": node.uid,
                        "name": node.name,
                ...
                # Convert edges to dictionaries for serialization
                for edge in edges:
                    all_edges.append({
                        "src": edge.src,
                        "dst": edge.dst,
                ...
                sys.stdout.write(f"\r[PROGRESS] [{'█' * (percentage // 2)}{'░' * (50 - percentage // 2)}] {files_processed}/{total_files} ({percentage}%)")
                sys.stdout.flush()
    ...
    logger.info(f"Saving AST data to {output_path}")
    ...
    logger.info(f"AST analysis complete. Data saved to {output_path}")
